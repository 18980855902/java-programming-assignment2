 ...
 * ID:20252307
 * Section: 1
 * Still have some problem with the method Mark and InValid
 * I cannot solve it.
 *
...
public class Sudoku {
    ...
    int[][] arrayTo2D(int [][][] array) {
        ...
        //just math.
        //Of course you can write this part in a more structure way like
        // if (box < 3)
        //      row = j;
        //  else if (box < 6)
        //      row = j + 3;
        //  else
        //      row = j + 6;
        // same for col. I actually like this writing more which is more clearer.
        return result;
    }
    /**
    ...
    boolean isValid(int[][][] cells) {
        int[] checkLineLogic = new int[18];
        int[][] checkBoxLogic = new int[3][3];
        for (int x = 0; x < 9; x++) {
            for (int y = 0; y < 3; y++) {
                for (int z = 0; z < 3; z++) {
                    checkBoxLogic[y][z] = cells[x][y][z];
                }
            }
            if (!checkBoxLogic(checkBoxLogic)) {
                System.out.print("1");
                return false;
            }
        }
        ...
        for (int l = 0; l < 9; l++) {
            if (l < 3) {
                for (int x = 0; x < 9; x++) {
                    if (x < 3) {
                        checkLineLogic[x] = cells[0][l % 3][x];
                    }
                    if (3 <= x && x < 6) {
                        checkLineLogic[x] += cells[1][l % 3][x - 3];
                    }
                    if (6 <= x) {
                        checkLineLogic[x] += cells[2][l % 3][x - 6];
                    }
                }
            }
            if (3 <= l && l < 6) {
                for (int x = 0; x < 9; x++) {
                    if (x < 3) {
                        checkLineLogic[x] = cells[3][l % 3][x];
                    }
                    if (3 <= x && x < 6) {
                        checkLineLogic[x] += cells[4][l % 3][x - 3];
                    }
                    if (6 <= x) {
                        checkLineLogic[x] += cells[5][l % 3][x - 6];
                    }
                }
            }
            if (6 <= l) {
                for (int x = 0; x < 9; x++) {
                    if (x < 3) {
                        checkLineLogic[x] = cells[6][l % 3][x];
                    }
                    if (3 <= x && x < 6) {
                        checkLineLogic[x] += cells[7][l % 3][x - 3];
                    }
                    if (6 <= x) {
                        checkLineLogic[x] += cells[8][l % 3][x - 6];
                    }
                }
            }
        }
        ...
        for (int l = 9; l < 18; l++) {
            if (l < 12) {
                for (int x = 9; x < 18; x++) {
                    if (x < 12) {
                        checkLineLogic[x] = cells[0][x % 3][l % 3];
                    }
                    if (12 <= x && x < 15) {
                        checkLineLogic[x] += cells[3][x % 3][l % 3];
                    }
                    if (15 <= x) {
                        checkLineLogic[x] += cells[6][x % 3][l % 3];
                    }
                }
            }
            if (12 <= l && l < 15) {
                for (int x = 9; x < 18; x++) {
                    if (x < 12) {
                        checkLineLogic[x] = cells[1][x % 3][l % 3];
                    }
                    if (12 <= x && x < 15) {
                        checkLineLogic[x] += cells[4][x % 3][l % 3];
                    }
                    if (15 <= x) {
                        checkLineLogic[x] += cells[7][x % 3][l % 3];
                    }
                }
            }
            if (15 <= l) {
                for (int x = 9; x < 18; x++) {
                    if (x < 12) {
                        checkLineLogic[x] = cells[2][x % 3][l % 3];
                    }
                    if (12 <= x && x < 15) {
                        checkLineLogic[x] += cells[5][x % 3][l % 3];
                    }
                    if (15 <= x) {
                        checkLineLogic[x] += cells[8][x % 3][l % 3];
                    }
                }
            }
        }
        if (!checkLineLogic(checkLineLogic)) {
            return false;
        }
    }
        return true;
    ...
    String filePicking() {
        ...
        System.out.println("Please input 1.txt");
    ...
    void printHelpMenu() {
        ...
        System.out.println("h\t\tOpen the help menu");
        System.out.println("c\t\tCheck the valid");
        System.out.println(".\t\tRemove the digit into the highlighted cells");
        System.out.println("a,s,d,w\tMove left,down,right, and up");
        System.out.println("number\tstuffing digit");

    }
    /**
     * To print the Sudoku in a nicer looking way.
     * 1. All horizontal borders should be printed with the char \u2500.
     * All vertical border should be printed with the char \u2502.
     * All cross-point should be printed with the symbol \u253c.
     *
     * 2. The highlighted cell, (at the position indicated by row and col),
     * should be \u25aa if it is empty or should be \u2081, \u2082,..., \u2089 depends on the value of that cell.
     * Please refer to the sample program and see how the program should work.
     *
    ...
    void advancePrint(int[][][] cells, int row, int col) {
        ...
        //this method is thinking out of the box isn't it?
   ...
   /**
    ...
    boolean same(int[][][]cells, int[][][]originals) {
        for (int x=0; x<9; x++){
            for (int y=0; y<3; y++){
                for (int z=0; z<3; z++){
                    if (originals[x][y][z]!=0 && cells[x][y][z]!=originals[x][y][z]){
                        return false;
                    }
                }
            }
